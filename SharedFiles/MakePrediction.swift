...
func makePrediction(modelManager: ModelManager,
    ...
    case .BicepCurl:
        let features = bicepCurlFeatureExtraction(buffer: motionManager.bufferCopy)
        ...
        (predictionProbs, prediction) = modelManager.bicepCurlPrediction(input: input) ?? ([:], "Other")

    case .LateralRaise:

        let features = lateralRaiseFeatureExtraction(buffer: motionManager.bufferCopy)
        ...
        (predictionProbs, prediction) = modelManager.lateralRaisePrediction(input: input) ?? ([:], "Other")
        
    case .ShoulderPress:

        let features = statisticalFeatureExtraction(buffer: motionManager.bufferCopy)
        ...
        (predictionProbs, prediction) = modelManager.shoudlerPressPrediction(input: input) ?? ([:], "Other")
        
    case .Exercise:

        let features = statisticalFeatureExtraction(buffer: motionManager.bufferCopy)

        let input = ExerciseInput(avg_result_accl: features.avg_result_accl ?? 0.0,
                                  avg_result_accl_fft: features.avg_result_accl_fft ?? 0.0,
                                  sma: features.sma ?? 0.0, sma_fft: features.sma_fft ?? 0.0,
        ...
        (predictionProbs, prediction) = modelManager.exercisePrediction(input: input) ?? ([:], "Other")
    }

    // Save the result.
    motionManager.results.append(prediction)
    
    // Only need to store the type of motion once each time the model is changed.
    if (motionManager.motionTypes.isEmpty) {
        motionManager.motionTypes.append("Good")
        motionManager.motionTypes.append("Other")
    }

    // Distribution of confidence across the different motion types in the model.
    motionManager.distribution.append([])
    motionManager.distribution.indices.last.map{ motionManager.distribution[$0].append(predictionProbs["Good"] ?? 0) }
    motionManager.distribution.indices.last.map{ motionManager.distribution[$0].append(predictionProbs["Other"] ?? 0) }
    
    for (key, value) in Array(predictionProbs).filter({$0.0 != "Good" && $0.0 != "Other"}).sorted(by: {$0.0 < $1.0}) {
        
        if (!motionManager.motionTypes.contains(key)) {
            motionManager.motionTypes.append(key)
        }
        motionManager.distribution.indices.last.map{ motionManager.distribution[$0].append(value) }
    }
}
