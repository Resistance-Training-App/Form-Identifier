...
class ModelManager: ObservableObject {
    ...
    let exercise: Exercise = {
        do {
            let config = MLModelConfiguration()
            return try Exercise(configuration: config)
        } catch {
            print(error)
    ...
    func exercisePrediction(input: ExerciseInput) -> (Dictionary<String, Double>, String)? {

        guard let modelPrediction = try? exercise.prediction(input: input)
