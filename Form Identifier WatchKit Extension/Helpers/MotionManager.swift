...
class MotionManager: ObservableObject {
    ...
    init() {
        // Queue for sample handling and calculations.
    ...
    func processDeviceMotion(_ deviceMotion: CMDeviceMotion) {
        ...
        DispatchQueue.global().async {
            ...
            if (self.currentIndexInPredictionWindow >= ModelManager.predictionWindowSize) {
                ...
                // Run form analysis model and count any potential reps in the buffer.
                processBuffer(modelManager: self.modelManager,
                              motionManager: self)

                // Temporary, only store one set of results.
